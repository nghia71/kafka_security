...
docker exec -it ${broker_container_name} bash -c \
    "cat /var/lib/kafka/data/${send_file} | /bin/kafka-console-producer --bootstrap-server ${broker_hostname}:${broker_local_port} --topic ${topic} --producer.config /client-creds/kafka-client.properties"
...
docker exec -it ${broker_container_name} /bin/kafka-console-consumer \
    ...
    --consumer.config /client-creds/kafka-client.properties > ${recv_file}
